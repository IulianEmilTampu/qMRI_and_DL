...
from tensorflow.keras.layers import Input, Conv2D, Activation, BatchNormalization, MaxPooling2D, Conv2DTranspose, Dropout, LeakyReLU
...
class unet(object):
    ...
    def __init__(self, img_size, Nclasses, class_weights, model_name='myWeightsAug.h5', Nfilter_start=64, depth=3, batch_size=3):
        ...
        def encoder_step(layer, Nf):
            ...
            x = Conv2D(Nf, (3,3), strides=2, kernel_initializer='he_normal', padding='same')(y)
            return y, x
...
## CUSTOM TRAINING FUNCTION
    def custum_train(self, train_gen, val_gen, train_steps, val_steps, max_epocs=300,
                    verbose=0, save_model_path=None, early_stopping=None, patience=None,
        ...
        # def dice(y_true, y_pred, class_weights, eps=1e-5):
        #
        #     tf_weights = tf.constant(class_weights[1::], dtype=tf.float32)
        #     num = tf.constant(2, dtype=tf.float32)*tf.reduce_sum(tf_weights*tf.cast(tf.reduce_sum(y_true[:,:,:,1::] * y_pred[:,:,:,1::], axis=range(1,len(y_true.shape)-1)),dtype=tf.float32))
        #     den = tf.reduce_sum(tf_weights*tf.cast(tf.reduce_sum(tf.math.square(y_true[:,:,:,1::]) + tf.math.square(y_pred[:,:,:,1::]),axis=range(1,len(y_true.shape)-1)),dtype=tf.float32))+eps
        #
        #     return num/den
        ...
        def grad(model, inputs, targets, class_weights):
            with tf.GradientTape() as tape:
                segmentation_loss = segmentationLoss(model, inputs, targets, class_weights, training=True)
        ...
        for epoch in range(self.maxEpochs):
            ...
            self.learning_rate = self.start_learning_rate * np.power(1-epoch/self.maxEpochs, 0.9)

            # ######## USING LOOKAHEAD OPTIMIZER
            optimizer = tf.keras.optimizers.Adam(learning_rate=self.learning_rate)
            # optimizer = tfa.optimizers.AdamW(learning_rate=self.learning_rate, weight_decay=0.0001)
            ...
            for x, y in train_gen:
                ...
                tr_epoch_accuracy.update_state(y.reshape((-1, 1)), self.model(x, training=False).numpy().reshape((-1,1)))
                tr_epoch_dice.append(dice(y, self.model(x, training=False), self.class_weights).numpy())
            ...
            for x, y in val_gen:
                ...
                val_loss = segmentationLoss(self.model, x, y, self.class_weights, training=False) + bceLoss(self.model, x, y, self.class_weights, training=False)
                # val_loss = segmentationLoss(self.model, x, y, self.class_weights, training=False)

                # track progress
                val_epoch_loss_avg.update_state(val_loss)
                val_epoch_accuracy.update_state(y.reshape((-1, 1)), self.model(x, training=False).numpy().reshape((-1,1)))
                val_epoch_dice.append(dice(y, self.model(x, training=False), self.class_weights).numpy())
            ...
            if epoch % 50 == 0:
                utilities.plotModelPerformance_v2(self.train_loss_history,
                                        self.train_accuracy_history,
                                        self.val_loss_history,
                                        self.val_accuracy_history,
                                        self.train_dice_history,
            ...
            if self.early_stopping:
                ...
                if self.val_dice_history[-1] > self.best_dice:
                    ...
                    self.model.save_weights(os.path.join(self.save_model_path, 'best_model_weights.tf'))
                ...
                if n_wait == self.patience:
                    if any([self.verbose == 1, self.verbose == 2]):
                        print(' -  Early stopping patient reached. Best model saved in {}'.format(self.save_model_path))
                    # save history training curves
                    json_dict = OrderedDict()
                    json_dict['training_loss'] = list(self.train_loss_history)
                    json_dict['validation_loss'] = list(self.val_loss_history)
