...
It uses the config.json file to load the right data for each fold and resumes the
...
- the fold has started training but did not finish -> resume training based on
    the last epoch trained. Only train for the remaining epochs by loading the last
...
if not su_debug_flag:
    ...
    parser.add_argument(
        ...
        required=True,
        help="Provide the folder where the cross validation folders are located for the model to resume training.",
    )
    parser.add_argument(
        "-cf",
        "--CROSS_VALIDATION_FILE",
        required=True,
        help="Provide the fpath to the configuration file created during the trianing initialization.",
    )
    parser.add_argument(
        "-dc",
        "--DATASET_CONFIGURATION",
        required=True,
        help="Which dataset configuration to resume training.",
    )
    parser.add_argument(
        "-df",
        "--IMG_DATASET_FOLDER",
        required=True,
        help="Provide the Image Dataset Folder where the folders for each modality are located. Default equal to the one set in the first training run (from the configuration file).",
    )
    parser.add_argument(
        "-af",
        "--ANNOTATION_DATASET_FOLDER",
        required=True,
        help="Provide the Annotation  Folder where annotations are located. Future versions will use the saved configuration file (Default equal to the one set in the first training run (from the configuration file)).",
    )
    parser.add_argument(
        "-bmf",
        "--BRAIN_MASK_FOLDER",
        required=True,
        help="Provide the Brain Mask Folder where annotations are located. Future versions will use the saved configuration file (Default equal to the one set in the first training run (from the configuration file)).",
    )
    parser.add_argument(
        "-gpu",
        "--GPU_NBR",
    ...
    parser.add_argument(
        ...
        "--MODEL_NAME",
        default="MyModel",
        help="Name used to save the model and the scores. Future versions will use the saved configuration file (Default equal to the one set in the first training run (from the configuration file)).",
    )
    ...
    parser.add_argument(
        ...
        "--BATCH_SIZE",
        default=3,
        help="Batch size. Future versions will use the saved configuration file (Default equal to the one set in the first training run (from the configuration file)).",
    )
    parser.add_argument(
        "-e",
        "--MAX_EPOCHS",
    ...
    parser.add_argument(
        ...
        "--RANDOM_SEED",
        default=29122009,
        help="Seed for the random choise of training, validation and testing subjects.",
    )
    ...
    args_dict["LEARNING_RATE"] = (
        float(args_dict["LEARNING_RATE"]) if args_dict["LEARNING_RATE"] else None
    )
    args_dict["BATCH_SIZE"] = (
        int(args_dict["BATCH_SIZE"]) if args_dict["BATCH_SIZE"] else None
    )
    args_dict["MAX_EPOCHS"] = (
        int(args_dict["MAX_EPOCHS"]) if args_dict["MAX_EPOCHS"] else None
    )
    args_dict["RANDOM_SEED"] = (
        int(args_dict["RANDOM_SEED"]) if args_dict["RANDOM_SEED"] else None
    )
    ...
    args_dict[
        "CROSS_VALIDATION_FOLDER"
    ] = "/home/iulta54/Code/P4-qMRI/trained_models/TEST_resume_training/Simple_2DUNet_rkv_10_fold5_lr0.001_batch3_cv_repetition_1_seed_1234/BRATS"
    args_dict[
        "CROSS_VALIDATION_FILE"
    ] = "/home/iulta54/Code/P4-qMRI/trained_models/TEST_resume_training/Simple_2DUNet_rkv_10_fold5_lr0.001_batch3_cv_repetition_1_seed_1234/train_val_test_subjects.json"
    args_dict["DATASET_CONFIGURATION"] = 0
    args_dict["IMG_DATASET_FOLDER"] = "/home/iulta54/Data/Gliom"
    args_dict["ANNOTATION_DATASET_FOLDER"] = "/home/iulta54/Data/Gliom/All_Annotations"
    args_dict["BRAIN_MASK_FOLDER"] = "/home/iulta54/Data/Gliom/All_Brain_masks"
    # args_dict['IMG_DATASET_FOLDER'] = None
    # args_dict['ANNOTATION_DATASET_FOLDER'] = None
...
if not os.path.isfile(args_dict["CROSS_VALIDATION_FILE"]):
    raise ValueError(
        f'The given configuration file path is not valid. Given {args_dict["CROSS_VALIDATION_FILE"]}'
    )
else:
...
devices = tf.config.list_physical_devices("GPU_NBR")

if devices:
    print(f'Running training on GPU_NBR # {args_dict["GPU_NBR"]} \n')
    warnings.simplefilter(action="ignore", category=FutureWarning)
else:
    Warning(
        f"ATTENTION!!! MODEL RUNNING ON CPU. Check implementation in case GPU_NBR is wanted."
    )
...
if not os.path.isdir(args_dict["CROSS_VALIDATION_FOLDER"]):
    raise ValueError(
        f'The given cross validation folder is not valid. Given {args_dict["CROSS_VALIDATION_FOLDER"]}'
    )

# get model name
args_dict["MODEL_NAME"] = os.path.basename(args_dict["CROSS_VALIDATION_FOLDER"])

folds_folders = glob.glob(
    os.path.join(args_dict["CROSS_VALIDATION_FOLDER"], "fold_*", "")
)

# build dictionary about status of each fold
summary_cv_training = {}
for idx, cv_f in enumerate(folds_folders):
    ...
    if os.path.isdir(os.path.join(cv_f, "last_model.tf")):
        ...
        summary_cv_training[idx]["best_model_path"] = os.path.join(
            cv_f, "best_model_weights.tf"
        )
...
for key, value in summary_cv_training.items():
    if value["finished_training"]:
        print(
            f'Fold {key+1}: Finished training (trained for {value["best_model_epoch"]} epochs)'
        )
    else:
        print(
            f'Fold {key+1}: To resume training (trained for {value["best_model_epoch"]} epochs)'
        )
...
for ids, subj in enumerate(subjects_to_load):
    ...
    aus_annotations_archive = utilities.load_3D_data(
        [os.path.join(args_dict["ANNOTATION_DATASET_FOLDER"], subj)]
    )
    ...
    for idx, modality in enumerate(modalities):
        ...
        file_name = os.path.join(
            ...
            modality,
            "_".join(modality.split("_")[1::]) + "_" + str(subj_idx) + ".nii.gz",
        )
...
for cv_f, cv_f_values in summary_cv_training.items():
    if not cv_f_values["finished_training"]:
        print(
            f'Resuming training fold {cv_f+1} (remaining epochs {args_dict["MAX_EPOCHS"] - cv_f_values["best_model_epoch"]})'
        )
        # run training fold
        # 1 - group training training, validation and testing data
        Xtest = [
            image_archive[int(subj.split("_")[-1].split(".")[0])]
            for subj in cross_validation_subjects["test"]
        ]
        Xtest = np.concatenate(Xtest, axis=0)

        Ytest = [
            annotation_archive[int(subj.split("_")[-1].split(".")[0])]
            for subj in cross_validation_subjects["test"]
        ]
        ...
        Xtrain = [
            image_archive[int(subj.split("_")[-1].split(".")[0])]
            for subj in cross_validation_subjects["train"][cv_f]
        ]
        ...
        Xvalid = [
            image_archive[int(subj.split("_")[-1].split(".")[0])]
            for subj in cross_validation_subjects["validation"][cv_f]
        ]
        ...
        # adjust learning rate start to match where the model stopped
        args_dict["LEARNING_RATE"] = args_dict["LEARNING_RATE"] * np.power(
            1 - cv_f_values["best_model_epoch"] / args_dict["MAX_EPOCHS"], 0.9
        )
        ...
        net_aug.custum_train(
            ...
            save_model_path=os.path.join(
                args_dict["CROSS_VALIDATION_FOLDER"], "fold_" + str(cv_f + 1)
            ),
            early_stopping=True,
            patience=args_dict["MAX_EPOCHS"] - cv_f_values["best_model_epoch"],
        ...
        if cv_f_values["best_model_path"]:
            ...
            cv_f_values["history"]["training_accuracy"].extend(
                net_aug.train_accuracy_history
            )
            json_dict["training_accuracy"] = cv_f_values["history"]["training_accuracy"]

            cv_f_values["history"]["validation_accuracy"].extend(
                net_aug.val_accuracy_history
            )
            json_dict["validation_accuracy"] = cv_f_values["history"][
                "validation_accuracy"
            ]
            ...
            with open(
                os.path.join(
                    args_dict["CROSS_VALIDATION_FOLDER"],
                    "fold_" + str(cv_f + 1),
                    "history.json",
                ),
                "w",
...
summary_file = os.path.join(
    args_dict["CROSS_VALIDATION_FOLDER"], f"tabular_test_summary.csv"
)
csv_file = open(summary_file, "w")
writer = csv.writer(csv_file)
csv_header = [
