...
def add_shadow_between_hues(ax, y_min, y_max, alpha=0.05, zorder=30, color="black"):
    ...
    lines = ax.get_lines()
    boxes = [c for c in ax.get_children() if type(c).__name__ == "PolyCollection"]
    lines_per_box = int(len(lines) / len(boxes))
...
SUMMARY_FILE_PATH = "/flush/iulta54/Research/P4-qMRI/Manuscript_results/Tumor_detection/overall_tabular_test_summary.csv"
SAVE_PATH = (
...
for metric_to_plot in metrics:
    ...
    # # # add hatches to  plot
    _ = [
        i.set_hatch(next(hatches))
        for i in box_plot.get_children()
    ...
    if save_images == True:
        file_name = f"overall_classification_summary_{model_version}_model_{metric_to_plot}_violin_plot"
        fig.savefig(
